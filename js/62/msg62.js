...
window.pcs.messageBox = (function () {
  ...
  function show(msg, modal, arrayOfButtons, callback) {
    ...
    if (!Array.isArray(arrayOfButtons)) {
      const okButton = document.createElement('button');
      okButton.innerHTML = 'ok';
      okButton.addEventListener('click', closebox);
      buttons.appendChild(okButton);
    } else {
      for (let i = 0; i < arrayOfButtons.length; i++) {
        const b = document.createElement('button');
        b.innerHTML = arrayOfButtons[i];
        b.addEventListener('click', () => {
          closebox();
          if (typeof callback === 'function') {
            callback(arrayOfButtons[i]);
          }
        });
    ...
    //         callback(arrayOfButtons[arrayOfButtons.findIndex(elem => elem === event.target)]);
    //     }
    //   }
    // });

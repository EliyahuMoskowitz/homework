(function () {
    ...
    const canvas = document.getElementById('theCanvas'), intervalLength = 16.66,// home = document.getElementById('home');
        ...
        homeLeftX = 100, homeLeftY = 70, homeLeftRadius = 50, midCanvas = 675, lengthOfPopUp = 10;// let foodRestPlace = 0;
            ...
            Ant.ants.forEach(a => {
                if (a.x === this.x && a.y === this.y && a.color !== this.color && !this.fighting) {
                    ...
                    if (this.strength > a.strength) {
                        this.strength += a.strength; console.log('this Winner has', this.strength, 'strength');
                        Ant.ants = Ant.ants.filter(ant => ant !== a);
                        a.isDead = true;
                    } else if (this.strength < a.strength) {
                        a.strength += this.strength; console.log('a Winner has', a.strength, 'strength');
                        Ant.ants = Ant.ants.filter(ant => ant !== this);
                        this.isDead = true;
                    } else {
                        let random = Ant.getRandomNumber(0, 1), remove;// ? this : a;
                        if (random) {
                            this.strength += a.strength; remove = a; console.log('this Winner has', a.strength, 'strength(was random)');
                        } else { a.strength += this.strength; remove = this; console.log('a Winner has', a.strength, 'strength(was random)'); }
